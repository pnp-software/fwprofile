...
/**
 * Create a control flow (other than the control flow from the initial node)
 * with the
 * given characteristics and add it to state machine.
 * The newly created control flow is stored in the control flow array at the
 * first non-null location
 * (in other words, the first control flow to be added to the state machine is
 * stored at location 0,
 * the second control flow is stored at location 1, etc).
 * @param prDesc the descriptor of the state machine to which the control flow
 * is added.
 * @param srcId the identifier of the source of the control flow.
 * @param srcType the type of the source.
 * @param destId the identifier of the destination of the control flow. If the
 * destination is an action
 * node, this argument has a positive value equal to the destination identifier.
 * If the destination
 * is a decision node, this argument has a negative value equal to the opposite
 * of the destination
 * identifier. If the destination is the final node, this argument has the value
 * zero.
 * @param cfGuard the control flow guard (or NULL if no guard is associated to
 * this control flow).
 */
 ...
 * - if the argument action is already present in the array of actions, the
 * function
 *   returns the index of the location where the action is stored;
 * - if the argument action is not present and the array of actions is not full,
 * the
 *   function adds it at the first free location and returns the index of this
 *   location;
 * - if the argument action is not present but the array of actions is already
 * full,
 ...
 * being equal to NULL and, in that case, it does not call
 * <code>AddAction</code>.
 ...
 * - if the argument guard is already present in the array of guards, the
 * function
 *   returns the index of the location where the guard is stored;
 * - if the argument guard is not present and the array of guards is not full,
 * the
 *   function adds it at the first free location and returns the index of this
 *   location;
 * - if the argument guard is not present but the array of guards is already
 * full, the
 *   function returns 0.
...
static void AddFlow(FwPrDesc_t prDesc, FwPrCounterS1_t srcId, NodeType_t srcType, FwPrCounterS1_t destId,
  ...
  /* New control flow will be stored in the control flow array of the Procedure
   * Descriptor at a location
   * in the range [baseLoc, baseLoc+nOfOutFlows-1]. We check that this range of
   * locations
   * is not already full (note that the checks performed when a node is added to
   * the procedure
   * ensure that the location [baseLoc+nOfOutFlows-1] is within the range of
   * flow array..
   */
  ...
  /* Identify location where newly added control flow will be stored in the
   * control flow array.
   * Note that the nOfOutFlows is guaranteed to be greater than zero by the way
   * it is
   * initialized in the previous statements in this function. Hence, the loop
   * will be
   * taken at least once. */
...
FwPrErrCode_t FwPrCheck(FwPrDesc_t prDesc) {
  ...
  /* Check that all control flow destinations are legal action nodes or decision
   * nodes */
  for (i = 0; i < prBase->nOfFlows; i++) {

...
For a decision node with $n$ outgoing control flows each outgoing control flow is assigned a distinct evaluation-order value
from the set $\{1,\ldots,n\}$ such that no two outgoing control flows
from the same decision node have the same evaluation-order. During procedure execution, guards out of a decision node are evaluated in increasing evaluation order.
The selected control flow is the one with the lowest evaluation-order whose guard evaluates to TRUE.
...
For a source with $n$ outgoing transitions, the evaluation order is a number in the set $\{1,\ldots,n\}$ such that no two transitions with the same source have the same evaluation order.

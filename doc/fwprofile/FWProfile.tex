...
\caption{Changes introduced in Revision 1.4.0}  \\
\hline
\rowcolor{light-gray}
...
\hline\hline
\endhead
3.2 & N & Added the previous state attribute to state machines. \\
\hline
3.2 & N & Added the previous node attribute to procedures. \\
\hline
all & E & Updated the documentation. \\
\hline
3.2, 3.6 & L & Clarified the order used to select a transition or control flow when two or more guards evaluate to true. \\
\hline
all & E & Replaced the figures with their updated versions. \\
\hline
\end{longtable}
...
For a decision node with $n$ outgoing control flows, where $n$ is a natural number, each outgoing control flow is assigned an evaluation-order value
from the set $\{1,\ldots,n\}$ according to the order in which the control flow was added to the procedure during the procedure configuration process.
Thus, the first control flow added has evaluation order 1 and the last control flow added to the same decision node has evaluation order $n$; no two outgoing control flows
from the same decision node have the same evaluation-order value. During procedure execution, the guards are evaluated in increasing evaluation order.
The selected control flow is the one with the lowest evaluation-order value whose guard evaluates to TRUE.
...
When a procedure starts, its previous node is set to an invalid value. During execution, 
when control flow enters an action node, the current node is copied to the previous node 
before being updated to the action node. Consequently, the previous node remains invalid 
upon leaving the initial node. For a self-transition, the previous and current nodes are identical. 
Finally, stopping a procedure resets its previous node and current node to an invalid value.
This is shown in the state diagram of Figure \ref{fig:PR_StartStop}.

\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.45,keepaspectratio=true]{../images/PR_StartStop.png}
...
the current node is an action node T, then: (a) the previous node is set equal to the current
node, (b) the current node is set equal to T, (c) the node execution counter is reset, (d) the
behaviour associated to T is executed, (e) the guard on the out-going control flow of T is evaluated
...
attached to the decision node are evaluated in their defined order; (b) if the target of the outgoing control
...
to TRUE is an action node T, then the previous node is set equal to the current node 
and the current node is set equal to T, the behaviour
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.38,keepaspectratio=true]{../images/PR_Execution.png}
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.43,keepaspectratio=true]{../images/PR_Example.png}
...
For a source with $n$ outgoing transitions, where $n$ is a natural number, let the evaluation-order function assign to each outgoing transition a distinct value
from the set $\{1,\ldots,n\}$ according to the order in which the transition was added to the state machine during the state machine configuration process.
Thus, the first transition added has evaluation order 1 and the last transition added from the same source has evaluation order $n$.
When a transition command is processed, let $M$ be the set of outgoing transitions whose trigger matches the command. The guards of the transitions in $M$
are evaluated in increasing evaluation order. The selected transition is the transition $t$ in $M$ with the minimum evaluation-order value such that the guard of $t$
evaluates to TRUE.
...
evaluated in their defined order and the actions associated to the transition with a guard evaluating to true is
...
When a state machine is started, its previous state is set to an invalid value. Before the
current state is changed during execution, its value is copied to the previous state. Consequently,
when the initial pseudo-state is left, the previous state remains invalid. For a transition to the
same state, the previous and current states are identical. When a state machine is stopped, its
previous state and current state are set to invalid values.
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.32,keepaspectratio=true]{../images/SM_StartStop.png}
 % PRD.png: 474x227 pixel, 72dpi, 16.72x8.01 cm, bb=0 0 474 227
...
trigger, then their guards are evaluated in their defined order. The absence of a guard is
equivalent to a guard that returns TRUE.
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.40,keepaspectratio=true]{../images/SM_CmdProcessing.png}
 % PRD.png: 474x227 pixel, 72dpi, 16.72x8.01 cm, bb=0 0 474 227
...
With reference to point 4, it Logic for Processing Transition Commands by a State Machineis noted that at least one of the guards on the outgoing transitions
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.35,keepaspectratio=true]{../images/SM_TransitionExecution.png}
 % PRD.png: 474x227 pixel, 72dpi, 16.72x8.01 cm, bb=0 0 474 227
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.42,keepaspectratio=true]{../images/SM_Example.png}
 % PRD.png: 474x227 pixel, 72dpi, 16.72x8.01 cm, bb=0 0 474 227
...
\begin{figure}[h]
 \centering
 \includegraphics[scale=0.35,keepaspectratio=true]{../images/FV_BasicApproach.png}
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.37,keepaspectratio=true]{../images/FV_Buffer.png}
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.4,keepaspectratio=true]{../images/FV_ActuatorController.png}
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.4,keepaspectratio=true]{../images/FV_Actuator.png}
 \caption{State Machines to Control the Hardware Actuator}
 \label{fig:FV_Actuator}
\end{figure}
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.44,keepaspectratio=true]{../images/FV_ActuatorController.png}

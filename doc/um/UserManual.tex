...
\item The previous state of the state machine
...
supported but their effect can be achieved as follows. If a choice pseudo-state has $n$ outgoing transitions, where $n$ is a natural number,
their unique evaluation-order values are members of the set $\{1,\ldots,n\}$ and are assigned according to the order in which they were added
to the state machine when it was configured. The transitions are evaluated in increasing evaluation order.
...
\item The previous node of the procedure
...
are evaluated in increasing evaluation order, where $n$ is the natural number of outgoing control flows and the evaluation-order values are assigned
from the set $\{1,\ldots,n\}$ according to the order in which the control flows were added to the procedure when the procedure was configured.
...
The previous state is set to an invalid value when the state machine is started. Before the current state is changed,
the current state is copied to the previous state. Thus, the previous state remains invalid when the initial pseudo-state is
left, and a transition to the same state makes the previous and current states equal. Stopping the state machine
sets both states to invalid values. If several transitions with the same trigger are available, their guards are
evaluated in the order in which the transitions were added to the state machine; the first transition with a true
guard is selected.
...
The previous node is set to an invalid value when the procedure is started. When execution enters an action node,
the current node is copied to the previous node before the current node is changed. Thus, the previous node remains
invalid when the initial node is left, and a self-transition makes the previous and current nodes equal. Stopping
the procedure sets both nodes to invalid values. If a decision node with $n$ outgoing control flows is encountered, where $n$ is a natural number,
each control flow is assigned an evaluation-order value from the set $\{1,\ldots,n\}$ according to the order in which it was added to the procedure.
The guards are evaluated in increasing evaluation order, and the control flow with the lowest evaluation-order value whose guard evaluates to TRUE
is selected.
...
which will be taken is the one with the lowest evaluation-order value. For a source with $n$ outgoing transitions, where $n$ is a natural number,
the evaluation-order values are unique members of the set $\{1,\ldots,n\}$ and are assigned according to the order in which the transitions were added
to the state machine. The guards of matching transitions are evaluated in increasing evaluation order, and the first guard which evaluates to TRUE
determines the selected transition.

Out-going transitions from a state or choice pseudo-state are therefore evaluated in increasing evaluation order. In the C1 Implementation, the transitions
from each source occupy a contiguous segment of the transition array in the state-machine descriptor. The segment is reserved when the source is configured,
and the \texttt{FwSmAddTrans*} functions in module \texttt{FwSmConfig.h} fill its slots in configuration order; the slot position therefore implements
...
which will be taken is the one with the lowest evaluation order. For a decision node with $n$ outgoing control flows, where $n$ is a natural number,
the evaluation-order values are unique members of the set $\{1,\ldots,n\}$, assigned according to the order in which the control flows were added
to the procedure. The guards are evaluated in increasing evaluation order, and the first guard which evaluates to TRUE determines the selected flow.

Out-going control flows from a decision node are therefore evaluated in increasing evaluation order. In the C1 Implementation, the control flows from each
decision node occupy a contiguous segment of the control-flow array in the procedure descriptor. The segment is reserved when the decision node is configured,
and the \texttt{FwPrAddFlow*} functions in module \texttt{FwPrConfig.h} fill its slots in configuration order; the slot position therefore implements
the evaluation order.

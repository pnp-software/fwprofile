...
(e.g. an action language). For a source with $n$ outgoing transitions, where $n$ is a natural number, each outgoing transition is assigned an evaluation-order value
...
Thus, the first transition added has evaluation order 1 and the last transition added to the same source has evaluation order $n$; no two outgoing transitions from the same source
have the same evaluation-order value. When a transition command is processed, the guards of matching outgoing transitions are evaluated in increasing evaluation order.
The selected transition is the one with the lowest evaluation-order value whose guard evaluates to TRUE. In the C1 Implementation, the outgoing transitions of each
state or choice pseudo-state occupy a contiguous segment of the transition array in the state-machine descriptor. The segment is reserved when the source is configured,
and calls to the appropriate \texttt{FwSmAddTrans*} function fill its slots in configuration order; consequently, the slot position implements the evaluation order.
Transition commands may carry parameters and may return values. The parameters and return values are not defined further by the
...
transitions from the choice pseudo-state are evaluated in their defined order until one is found

...
(e.g. an action language). The evaluation order of transitions with the same source is the order in which they were added to the state machine during the state machine configuration process. If the guards of two or more such transitions evaluate to TRUE, the first transition in this order is selected. Transition commands may carry parameters and may return values. The parameters and return values are not defined further by the

...
evaluates to true, the transition which will be executed is the one whose guard is evaluated first. The guards are evaluated in the order in which
their transitions were added to the state machine during the state machine configuration process.
...
When a procedure must traverse a decision node, it evaluates the guards of all out-going control flows from the decision node in the order in which
their control flows were added to the procedure during the procedure configuration process and selects the first one which has a true guard.
